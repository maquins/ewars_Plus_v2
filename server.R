...
server<-function(input,output,session) { 
  ...
  source("New_model_server_Script.R",local =T,echo =F)
 

  #eval(parse(text=readLines("Spatial_temporal.R")))
  source("Spatial_temporal.R",local =T,echo =F)
  
  source("DBII_Update_District_Input.R",local =T)

  #eval(parse(text=readLines("Reactive_function_DBII_New_Model.R")))
  source("DBII_Prospective_Predictions.R",local =T)
